...
#include "LedManager.h"    // Für LedManager und LedState
...
void WebSocketClient::processCommand(const String& message) {
    ...
    } else if (command == "set_led_state") {
        // LED-Zustand-Befehl an LedManager weiterleiten
        String state = doc["state"] | "";
        Serial.printf("WebSocketClient: LED-Zustand-Befehl - State: %s\n", state.c_str());
        
        // Externer LedManager-Zugriff
        extern LedManager ledManager;
        
        if (state == "thinking") {
            ledManager.setState(LedState::THINKING);
            Serial.println("WebSocketClient: LED-Zustand auf THINKING gesetzt");
        } else if (state == "idle") {
            ledManager.setState(LedState::IDLE);
            Serial.println("WebSocketClient: LED-Zustand auf IDLE gesetzt");
        } else if (state == "listening") {
            ledManager.setState(LedState::LISTENING);
            Serial.println("WebSocketClient: LED-Zustand auf LISTENING gesetzt");
        } else if (state == "playing") {
            ledManager.setState(LedState::PLAYING);
            Serial.println("WebSocketClient: LED-Zustand auf PLAYING gesetzt");
        } else if (state == "error") {
            ledManager.setState(LedState::ERROR);
            Serial.println("WebSocketClient: LED-Zustand auf ERROR gesetzt");
        } else {
            Serial.printf("WebSocketClient: Unbekannter LED-Zustand: %s\n", state.c_str());
        }
    }
}
